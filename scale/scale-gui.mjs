...
import { ScaleElementMVC } from '/HafrenHaverJS/scale/element/scale-element-mvc.mjs';
import { SCALE_DB        } from '/HafrenHaverJS/scale/scale-model.mjs';

export class ScaleGUI extends ElementalCanvasGUI {
	constructor(id) {
		super(id);
	...
	addElements() {
		...
		for(var i = 0; i < len; i++) {
			...
			const index  = (i + 8) % len;
			const element = new ScaleElementMVC(this, this.w2, this.h2, r2, theta, this.roff, index);
	...
	drawIndicatorArrow() {
		...
		const entry2  = SCALE_DB[index2 % len];
		const r3      = r - roff;
